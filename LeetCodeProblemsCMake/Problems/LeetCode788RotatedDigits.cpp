...
int dp(vector<int> &digits, unordered_set<int> &validDigits, unordered_set<int> &flippingDigits,
       vector<vector<vector<int> > > &memo, int pos, int tight,
       int flipped) {
...
int LeetCode788RotatedDigits::rotatedDigits(int n) {
    ...
    vector<vector<vector<int> > > memoisation = vector<vector<vector<int> > >(
        length, vector<vector<int> >(2, vector<int>(2, -1)));
