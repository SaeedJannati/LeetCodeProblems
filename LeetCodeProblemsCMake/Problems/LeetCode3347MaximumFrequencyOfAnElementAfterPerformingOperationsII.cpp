...
int LeetCode3347MaximumFrequencyOfAnElementAfterPerformingOperationsII::maxFrequency(vector<int> &nums, int k,
    ...
    for (int i = 0; i < uniquePointer; i++) {
        ...
        currentFrequency = frequencies[nums[i]] + min(static_cast<long long>(numOperations),
                                                      prefixSum[right] - prefixSum[left] + frequencies[nums[right]] -
                                                      frequencies[nums[i]]);
    ...
    for (int i = 0; i < uniquePointer - 1; i++) {
        ...
        currentFrequency = min(static_cast<long long>(numOperations),
                               prefixSum[right] - prefixSum[i] + frequencies[nums[right]]);
        maxFrequency = max(maxFrequency, currentFrequency);
    }
    return maxFrequency;
}
